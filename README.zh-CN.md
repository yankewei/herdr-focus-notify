...
未点击的通知会保留 **5 分钟**，之后由 `alerter` 自动消除。

设这个上限是因为 `alerter` 在等待期间会持续增长常驻内存——实测约 12.9 MB/分钟，且没有平台期。只有一条通知时看不出来，没有上限时则会迅速累积：Agent 在你不在电脑前时陆续完成，每个 pane 都会堆一个等待进程，各自持续增长，直到被点击或超时。
...
如果你需要更长的通知存活时间，并且能接受这部分内存开销，可以用 `HERDR_FOCUS_NOTIFY_TIMEOUT_SECS`（单位为秒）覆盖：

```bash
HERDR_FOCUS_NOTIFY_TIMEOUT_SECS=900 herdr
```

设为 `0` 会完全关闭超时，通知会一直保留到被点击——同时泄漏也不再有上限。
